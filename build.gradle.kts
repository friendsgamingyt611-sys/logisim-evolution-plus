...
application {
  applicationName = "logisim-evolution+"
...
val APP_EXEC_NAME = "appExecName"
val UPPERCASE_APP_EXEC_NAME = "uppercaseAppExecName"
...
extra.apply {
  ...
  val appExecName = "logisim-evolution+"
  set(APP_EXEC_NAME, appExecName)
  val uppercaseAppExecName = appExecName.replaceFirstChar { it.uppercase() }.trim()
  set(UPPERCASE_APP_EXEC_NAME, uppercaseAppExecName)

  // Base name of produced artifacts. Suffixes will be added later by relevant tasks.
  val baseFilename = "${appExecName}-${appVersion}"
  set(TARGET_FILE_PATH_BASE, "${targetDir}/${baseFilename}")
  logger.debug("targetFilePathBase: \"${targetDir}/${baseFilename}\"")
  val baseFilenameShort = "${appExecName}-${appVersionShort}"
  ...
  set(APP_DIR_NAME, "${buildDir}/macOS-${osArch}/${uppercaseAppExecName}.app")
}
...
tasks.register("createDeb") {
  ...
  val appExecName = ext.get(APP_EXEC_NAME) as String
  ...
  val outputFile = "${targetDir}/${appExecName}_${appVersion}_${debArch}.deb"
...
tasks.register("createMsi") {
  ...
  doLast {
    ...
    val fromFile = "${targetDir}/${appExecName}-${version}.msi"
    val toFile = "${targetDir}/${appExecName}-${version}-${osArch}.msi"
...
tasks.register("createExe") {
  ...
  doLast {
    ...
    func.verifyFileExists("${dest}/${appExecName}/${appExecName}.exe")
  }
}
...
tasks.register<Zip>("createWindowsPortableZip") {
  ...
  archiveFileName = "${appExecName}-${version}-windows-${osArch}.zip"
...
tasks.register("createApp") {
  ...
  val projectName = ext.get(UPPERCASE_APP_EXEC_NAME) as String
...
//    }
  ...
  shadowJar {
    val appExecName = ext.get(APP_EXEC_NAME) as String
    archiveBaseName.set(appExecName)
    archiveVersion.set(ext.get(APP_VERSION) as String)
    from(".") {
      include("LICENSE.md")
      include("README.md")
