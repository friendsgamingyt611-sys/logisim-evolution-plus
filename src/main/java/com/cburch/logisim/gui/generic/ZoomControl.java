...
public class ZoomControl extends JPanel {
  ...
  private final ZoomField zoomField;
  ...
  public ZoomControl(ZoomModel model, Canvas canvas) {
    ...
    zoom.add(zoomField, BorderLayout.CENTER);
    ...
    model.addPropertyChangeListener(ZoomModel.ZOOM, zoomField);
  }
  ...
  public String zoomString() {
    ...
    final var percent = model.getZoomFactor() * 100.0;
    final var text = df.format(percent);
    return text + "%";
  }

  private void setZoomFactor(double value) {
    final var mousePos = canvas.getMousePosition();
    if (mousePos != null) {
      model.setZoomFactor(value, mousePos.x, mousePos.y);
    } else {
      model.setZoomFactor(value);
    }
  }
  ...
  public void zoomOut() {
    ...
    for (var i = choices.size() - 1; i >= 0; i--) {
      if (choices.get(i) < factor) {
        setZoomFactor(choices.get(i) / 100.0);
        break;
      }
    }
  }
  ...
  public void setZoomModel(ZoomModel value) {
    ...
    if (oldModel != value) {
      if (oldModel != null) {
        ...
        oldModel.removePropertyChangeListener(ZoomModel.ZOOM, zoomField);
      }
      model = value;
      if (value == null) {
        ...
        value.addPropertyChangeListener(ZoomModel.ZOOM, zoomField);
        zoomField.setText(zoomString());
      }
    }
  }
  ...
  private class ZoomField extends JTextField implements PropertyChangeListener, ActionListener, FocusListener {
    private static final long serialVersionUID = 1L;

    public ZoomField() {
      super(zoomString(), 5);
      setHorizontalAlignment(SwingConstants.CENTER);
      setToolTipText("Type zoom percentage and press Enter");
    ...
    private void commitText() {
      ...
      final var text = getText().trim();
      final var raw = text.endsWith("%") ? text.substring(0, text.length() - 1).trim() : text;
      try {
  ...
  public class PredefinedZoomButton extends JButton implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      if (zoomModel != null && canvas.getProject().getCurrentCircuit() != null) {
        final var mousePos = canvas.getMousePosition();
        if (mousePos != null) {
          zoomModel.setZoomFactor(zoomValue, mousePos.x, mousePos.y);
        } else {
