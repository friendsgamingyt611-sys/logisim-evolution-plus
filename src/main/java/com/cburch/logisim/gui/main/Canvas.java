...
public class Canvas extends JPanel implements LocaleListener, CanvasPaneContents, AdjustmentListener {
  ...
  private void doZoom(Point mouseLocation, boolean zoomIn) {
    final var model = proj.getFrame().getZoomModel();
    if (model == null) {
      return;
    }
    ...
    final var factor = zoom * 100.0 * (zoomIn ? 1.001 : 0.999);
    double newZoom = zoom;
    if (zoomIn) {
      for (final var choice : choices) {
        if (choice > factor) {
          newZoom = choice / 100.0;
          break;
        }
      }
    } else {
    ...
    if (mouseLocation != null && model instanceof BasicZoomModel bzm) {
      bzm.setZoomFactor(newZoom, mouseLocation.x, mouseLocation.y);
    } else {
  ...
  private class MyListener
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if (e.getButton() == MouseEvent.BUTTON1
        ...
        if (dragTool != null) {
          ...
          if (e.getButton() != MouseEvent.BUTTON1 && !(dragTool instanceof MenuTool)) {
    ...
    public void mouseWheelMoved(MouseWheelEvent mwe) {
      final var tool = proj.getTool();
      if (tool instanceof PokeTool && ((PokeTool) tool).isScrollable()) {
        ...
        // Continuous zoom centered on mouse using precise wheel rotation
